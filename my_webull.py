...
class my_webull :
    ...
    def get_calendar(self, stock=None, tId=None, timeStamp=None):
        '''
        ...
        response = requests.get(self._urls.bars(tId,timestamp=timeStamp), params=params, headers=headers, timeout=self.timeout)
        result = response.json()
        #logging.info(result)
        time_zone = timezone(result[0]['timeZone'])
...
''' Paper support '''
class my_paper_webull(my_webull):
    ...
    def modify_order_option(self, order=None, lmtPrice=None, stpPrice=None, enforce=None, quant=0):
        ...
        headers = self.build_req_headers(include_trade_token=True, include_time=True)
        data = {
            'oderId': order[0]['orderId'],
            'orderType': order[0]['orderType'],
            'accountId': self._account_id,
            'paperId': 1,
            'timeInForce': enforce or order[0]['timeInForce'],
            'serialId': str(uuid.uuid4()),
            'orders': [{'quantity': quant or order[0]['totalQuantity'],
                        'action': order[0]['action'],
                        'tickerId': order[0]['ticker']['tickerId'],
                        'tickerType': 'OPTION',
    ...
    def place_order_otoco_option(self, optionId='', price='', stop_loss_price='', limit_profit_price='', time_in_force='DAY', quant=0) :
        '''
        OTOCO: One-triggers-a-one-cancels-the-others, aka Bracket Ordering
        Submit a buy order, its fill will trigger sell order placement. If one sell fills, it will cancel the other
         sell
        ...
        data1 = {
            'newOrders': [
                {'orderType': 'LMT', 'timeInForce': time_in_force, 'quantity': int(quant),
                 'outsideRegularTradingHour': False, 'action': 'BUY', 'tickerId': int(optionId),
                 'lmtPrice': float(price), 'comboType': 'MASTER'},
                {'orderType': 'STP', 'timeInForce': time_in_force, 'quantity': int(quant),
                 'outsideRegularTradingHour': False, 'action': 'SELL', 'tickerId': int(optionId),
                 'auxPrice': float(stop_loss_price), 'comboType': 'STOP_LOSS'},
                {'orderType': 'LMT', 'timeInForce': time_in_force, 'quantity': int(quant),
                 'outsideRegularTradingHour': False, 'action': 'SELL', 'tickerId': int(optionId),
                 'lmtPrice': float(limit_profit_price), 'comboType': 'STOP_PROFIT'}
        ...
        response1 = requests.post(url, json=data1, headers=headers, timeout=self.timeout)
        result1 = response1.json()
        ...
        if result1['forward'] :
            ...
            response2 = requests.post(url, json=data2, headers=headers, timeout=self.timeout)
